...
class ClusterObserver:
    ...
    # --- DS-side status verification (via admin RPC) ---
    ...
    def get_ds_status(self, ds_ip: str, ds_admin_port: int) -> dict[str, str]:
        """Query DS internal status via admin RPC.
        Returns {"is_registered": "true/false", "cm_ready": "true/false",
                 "heartbeat_failure_count": "N"}.
        """
        try:
            return self._admin.get_ds_status(ds_ip, ds_admin_port)
        except AdminClientError:
            return {}

    def assert_ds_is_registered(self, ds_ip: str, ds_admin_port: int) -> None:
        """Assert DS reports itself as registered with CM."""
        status = self.get_ds_status(ds_ip, ds_admin_port)
        assert status.get("is_registered") == "true", (
            f"DS {ds_ip}:{ds_admin_port} is_registered={status.get('is_registered')}, "
            f"expected true"
        )

    def assert_ds_cm_ready(self, ds_ip: str, ds_admin_port: int,
        ...
        assert status.get("cm_ready") == expected_str, (
            f"DS {ds_ip}:{ds_admin_port} cm_ready={status.get('cm_ready')}, "
            f"expected {expected_str}"
        )

    def assert_ds_heartbeat_failure_count(
        self, ds_ip: str, ds_admin_port: int, min_count: int = 0,
        ...
        if min_count > 0:
            assert count >= min_count, (
                f"DS {ds_ip}:{ds_admin_port} heartbeat_failure_count={count}, "
    ...
    def wait_for_ds_cm_not_ready(
        self, ds_ip: str, ds_admin_port: int,
        timeout: float = 60, poll_interval: float = 1.0
    ) -> bool:
        ...
        while time.time() < deadline:
            ...
            if status.get("cm_ready") == "false":
                logger.info("DS %s:%d reports cm_ready=false", ds_ip, ds_admin_port)
                return True
            time.sleep(poll_interval)
        logger.warning("Timed out waiting for DS %s:%d cm_ready=false", ds_ip, ds_admin_port)
        return False
    ...
    def wait_for_ds_registered(
        ...
        while time.time() < deadline:
            ...
            if (status.get("is_registered") == "true"
                ...
                logger.info("DS %s:%d registered and cm_ready", ds_ip, ds_admin_port)
                return True
            time.sleep(poll_interval)
        logger.warning("Timed out waiting for DS %s:%d registration", ds_ip, ds_admin_port)
        return False
